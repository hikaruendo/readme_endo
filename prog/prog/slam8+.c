...
double MAPy=111319.490793274;//(6378137*D2R);
double MAPx=92396.5456539047;//(cos(33.9*D2R)*40075017/360);
...
double Vmu[3]={0,0,0};//for noize in prediction of position
double Xmu[3]={0,0,0};//for noize in prediction of position

double PERCENT_RE=0.3;
...
int main(int argc, char *argv[]){
  ...
  int i;
  double S_map=8,S_ors=16;//実験で求めるガウス関数の幅
  int totaltime0=10000;//途中で確かめるため
  double roadwidth=3;
  //  double AMO[2]={1.0,0};
  double A_map=1.0,A_ors=0;
  for(i=0;i<argc;i++){
    if(strncmp(argv[i],"NP:",3)==0){
      ...
      sscanf(&argv[i][6],"%lf:%lf:%lf:%lf:%lf:%lf",&sigma_v,&sigma_w,&sigma_g,&Vmu[0],&Vmu[1],&Vmu[2]);
    }
    ...
    else if(strncmp(argv[i],"ft:",3)==0){//理想軌道 map gpst2017-2.dat
      FILEDATA *F=&(par->F[0]);
      char *p=&argv[i][3];
    ...
    /*************kinect追加***************/
      ...
      sscanf(&argv[i][6],"%d:%d",&input.type,&_2Dor3D);
    }
    else if(strncmp(argv[i],"xysigma:",8)==0){
      sscanf(&argv[i][8],"%lf:%lf:%lf:%lf:%lf:%lf",&sigma_x,&sigma_y,&sigma_theta,&Xmu[0],&Xmu[1],&Xmu[2]);
    }
    else if(strncmp(argv[i],"S:",2)==0){
      sscanf(&argv[i][2],"%lf:%lf",&S_map,&S_ors);
    }
    else if(strncmp(argv[i],"Smap:",5)==0){
      sscanf(&argv[i][5],"%lf",&S_map);
    }
    else if(strncmp(argv[i],"A:",2)==0){
      sscanf(&argv[i][2],"%lf:%lf",&A_map,&A_ors);
    }
    else if(strncmp(argv[i],"tt:",3)==0){
      sscanf(&argv[i][3],"%d",&totaltime0);
    }
    else if(strncmp(argv[i],"RS:",3)==0){
      sscanf(&argv[i][3],"%lf",&PERCENT_RE);
    }
    else if(strncmp(argv[i],"rw:",3)==0){
      sscanf(&argv[i][3],"%lf",&roadwidth);
    }
  }
  //printf("par->robotang0=%lf\n",par->robotang0);
  //exit(0);
...
//  }
  //設定内容を確認，実行許可を得る//file open?
    int totaltime=makesuretouser(par);//in _slam.c open par->ACTfp <==>open U(odometry),F(gps),K(kinect)
  ...
  if((T->fp=fopen(T->fn,"r"))==NULL){
    printf("\n\nERROR:CAN'T OPEN T-FILE (%s) :CHECK THE argv[1]\n",T->fn);
    exit(-1);
  }
  rewind(T->fp);
  ...
  if(((F->fp=fopen(F->fn,"r"))==NULL)&&(par->ft==1)){
    printf("\n\nERROR:CAN'T OPEN F-FILE (%s) :CHECK THE argv[1]\n",F->fn);
    exit(-1);
  }
  rewind(F->fp);
  double *map_xt;
  ...
  double mapAA;
  double disAA;
  map_xt = (double *)malloc(sizeof(double)*F->ndata);
  map_yt = (double *)malloc(sizeof(double)*F->ndata);
  int n;
  for(n=0;n<F->ndata;n++){
    fgets(F->fn,126,F->fp);
    int ret_f=sscanf(F->fn,"%lf %lf\n",&map_yt[n],&map_xt[n]);
    if(n!=0){
      map_xt[n]=(map_xt[n]-map_xt[0])*MAPx;
      map_yt[n]=(map_yt[n]-map_yt[0])*MAPy;
    }
  ...
  if(DELTA<U->T) DELTA=U->T;//max ZU->Ty
  {
    //int n;
    for(n=0;n<par->nZ;n++){
      FILEDATA *Z=&(par->Z[n]); //LRF,GPS,ORS
      rewind(Z->fp); 
      Z->l=0;
...
//  int spa_img_w=200;
  ...
  //  totaltime=100;
  if(totaltime0<totaltime) totaltime=totaltime0;
  for(i=0;i<totaltime;i++){//totaltime=par->Z[0].ndata// number of GPS data
    ...
    if(T->fp){
      ...
      if(feof(T->fp)) break;
      gpstime=i_min_sec_usec(min,sec,usec);//gpstime= min*1000000+sec*1000 + usec;
    }
    else gpstime=0;
    // printf("gpstime=%d\n",gpstime);
    switch(input.type){
    case 1:
      getInput1(&input,U,gpstime,i);//defined in fileRorW5.c
      break;
    case 2:
      //      DEBUG=1;
      getInput2(&input,U,gpstime,i);//defined in fileRorW5.c
      //      DEBUG=0;
      //      exit(0);
      break;
    default:
    ...
    predict_pos(input,pos_k,pos_predict);//pfprocess.c //done
	...
	if(input.type==2){
	  ...
	  for(j=0;j<NofParticles;j++){
	    double wj_ors=0;
	    if(F->fn){
	      double a=pos_predict[j].state.theta;//no
#define ORS_NEW
#ifdef ORS_NEW
              double angORSorig=angORS;
	      angORS=atan2(sin(angORS*D2R),-cos(angORS*D2R))*R2D;//
              double da0=(a-angORS)*D2R;
	      double da=fabs(atan2(sin(da0),cos(da0)))*R2D;
#else
	      double da=fabs(a-angORS);//no
	      //double da_gps=fabs(a-gps.theta);
	      if(da>fabs(a-angORS1)) da=fabs(a-angORS1);//no
	      else if(da>fabs(a-angORS2)) da=fabs(a-angORS2);//no
#endif//#ifdef ORS_NEW
              if(j<5){
                printf("da=%g=|angORS(%g)-theta(%g)|=%g,angORSorig=%g\n",da,angORS,a,da0*R2D,angORSorig);
  	      }
	      wj_ors=exp(-da*da/S_ors)+smallw_ors;//no
	    }
	    //	    double px=pos_predict[j].state.pos.x;
	    //	    double py=pos_predict[j].state.pos.y;
	    double px=pos_predict[j].state.pos.x/m2mm;
	    double py=pos_predict[j].state.pos.y/m2mm;
	    double pz=pos_predict[j].state.pos.z/m2mm;
	    double dis_map_orig=calcalfa(F,map_xt,map_yt,px,py);//kinect計測値と理想軌道との距離
	    //double dis_map=calcalfa_k(F,map_xt,map_yt,px,py);//kinect計測値と理想軌道との距離
	    double dis_map=calcalfa_k3d(F,map_xt,map_yt,px,py,pz);//kinect計測値と理想軌道との距離
            if(_2Dor3D==2){
	      dis_map=calcalfa_k2d(F,map_xt,map_yt,px,py);//kinect計測値と理想軌道との距離
	    }
            else{//2d
	      dis_map=calcalfa_k3d(F,map_xt,map_yt,px,py,pz);//kinect計測値と理想軌道との距離
	    }
	    double wj_map;//
	    //	    double wroad=1;//width of road 1500mm
	    double mapA;//?
	    //	    S_map=2*5*5.;
	    //	    S_map=2*2*2.;
	    if(dis_map<=roadwidth){//less than or equal to 3m
              wj_map=1;
	    }
	    else {
              double s=8;//sigma
              double dr=dis_map-roadwidth;
              wj_map=(exp(-dr*dr/S_map)+0.1)/1.1;
	      //              wj_map+=2.0*(exp(-pz*pz/S_map)+0.1);
	      //              printf("wj_map%g dr%g=%g-%g\n",wj_map,dr,dis_map,roadwidth);
	      // mapA=((sqrt(2*sigma*PI))*(exp(-dis2_map/16.0)/(sqrt(16.0*PI))))/(8*sqrt(PI));
	      //kinectA=((sqrt(16.0*PI))*(exp(-dis_true*dis_true/16.0)/(sqrt(16.0*PI))))/(8*sqrt(PI));
	    }
	    mapA=A_map;
	    mapAA=mapA;
	    orsA=A_ors;
	    //            printf("A=%g,%g",mapA,orsA);

	    double wj=(wj_ors * orsA) + (wj_map * mapA);
            if(j<=5){
              printf("#i%d j%d p(%.1f,%.1f) map(%.1f,%.1f),(%.1f,%.1f),(%.1f,%.1f),(%.1f,%.1f) ",i,j,px,py,map_xt[0],map_yt[0],map_xt[1],map_yt[1],map_xt[2],map_yt[2],map_xt[3],map_yt[3]);
	      printf("dis%.1f=?=%.1f,wj%g\n",dis_map,dis_map_orig,wj_map);
	    }
	  ...
	  for(j=0;j<NofParticles;j++){
	    ...
	    double dis_true=calcalfa(F,map_xt,map_yt,kinect.x,kinect.y);//kinect計測値と理想軌道との距離
	    disAA=dis_true;
    ...
    ///////////////////////
    //#ifdef ATODE
    resampling(ess_file,pos_predict,pos_resample,i);//resampling 
    ...
    recordRobotstateALL(pos_file,robot,i);//position_usingPF.dat
    ...
    recordweightdata(weight_file,mapAA,i);
    recordtruedisdata(true_dis_file,disAA,gps.nS,i);
#define ATODE
#ifdef ATODE    
    double par_map_dis=calcalfa(F,map_xt,map_yt,robot.pos.x/1000.0,robot.pos.y/1000.0);
    //recordtruedisdata(dis_particle_map_file,par_map_dis,gps.nS,i);
    double onlyu_map_dis=calcalfa(F,map_xt,map_yt,nonnoisepos.pos.x/1000.0,nonnoisepos.pos.y/1000.0);
	...
	//debug        printf("%.1f %.1f %.1f %d\n",particle.pos.x,particle.pos.y,particle.theta,j);
