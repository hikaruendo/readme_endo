...
void recordInitrobotstate(FILE* fp,double ang){
  fprintf(fp,"#0.0000 0.0000 %lf -1 0.000 #x,y,theta,t,z\n",ang);//modified by kuro ???recordRobotstateALL
  //  fprintf(fp,"#0.0000 0.0000 %lf -1 0 #x,y,z,ax,ay,az,t\n",ang);//modified by kuro ???
...
void recordRobotstateALL(FILE* fp, STATE state, int t){
  fprintf(fp,"%.3f %.3f %.3f %d %.3f\n",state.pos.x,state.pos.y,state.theta,t,state.pos.z);//modified by kuro
...
char *getInput(INPUT *input,FILEDATA *U,int gpstime,int loopnum){
  char buff[buffsize];
  char *ret;
  int lD=0,arraynum,flag=0,odofirst=0;
  long int odotime;
  double timedif=0.0;
  int dif[2];
  fpos_t fpos;
...
#define READ_U_NEW
  ...
  for(;;){//do wile gpstime>odotime ??
    ...
    double tmp;
     int sec,usec,min;
    double tsecf;//sec
    sscanf(buff,"%lf",&tsecf);//kinect 3D
    if(input->t00<0){
    //    if(U->t00<0) U->t00=-tsecf+45*60;
      if(gpstime>0) input->t00=tsecf-gpstime/1000.;
      else input->t00=-tsecf;
      printf("t00=%d?\n",gpstime);
    }
    tsecf-=input->t00;
    min=(int)(tsecf/60);
    sec=(int)(((int)tsecf)%60);
    usec=(int)(tsecf*1000)%1000;
    odotime=tsecf*1000.;//convert from sec to usec
    printf("0tsecf,min,sec,usec=%.3f,%d,%d,%d\n",tsecf,min,sec,usec);
    if(gpstime<odotime) break;
    if(gpstime<=0) break;//read only one if gps is not used
  }
#else //#ifdef READ_U_NEW
  for(;;){
    //  if(lD==22) exit(0);
    ...
    if(input->t00<-1e10){
      ...
      //      printf("t00=%.3f=%.3f-%.3f\n",input->t00,gpstime/1000.,tsecf);
    }
    tsecf-=input->t00;
    min=(int)(tsecf/60);
    sec=(int)(((int)tsecf)%60);
    usec=(tsecf-(int)tsecf)*1000;
    //    usec=(int)(tsecf*1000)%1000;
    odotime=i_min_sec_usec(min,sec,usec);
    printf("0tsecf,min,sec,usec=%.3f,%d,%d,%d\n",tsecf,min,sec,usec);
      //    odotime=odotime_d*1000.;//convert from sec to usec
    if(gpstime<=0){
      lD=1;
      break;//read only one if gps is not used
    }
    if(lD!=0){
      arraynum =(lD + 1) % 2;
      switch(arraynum){
  ...
  if(U->class==1){//
    ...
    for(l=0;l<lD;l++){
      ret=fgets(buff,buffsize,U->fp); 
      if(feof(U->fp)) break;
      sscanf(buff,"%lf%lf%lf%lf",&tsecf,&_X[1],&_X[0],&_X[2]);
      _X[0]*=-1;
      tsecf-=input->t00;
      //      sscanf(buff,"%lf%lf%lf %lf:%d:%d.%d\n",&_x,&_y,&_a,&tmp,&min,&sec,&usec);
      min=(int)(tsecf/60);
      ...
      usec=(tsecf-(int)tsecf)*1000;
      //      usec=(int)(tsecf*1000)%1000;
      printf("1tsecf,min,sec,usec=%.3f,%d,%d,%d\n",tsecf,min,sec,usec);
      for(i=0;i<U->nv;i++) _X[i]*=-1000;
      for(i=0;i<U->nv;i++) X[i]+=_X[i];
      U->l++;
      if(DEBUG!=0){
        printf("DEBUG0(%d):_X(%g,%g,%g),X(%g,%g,%g)",odotime,_X[0],_X[1],_X[2],X[0],X[1],X[2]);
      }
    }

    odoend = U->timeA = i_min_sec_usec(min,sec,usec);//min*1000000+sec*1000 + usec;
    printf("odoend=%d,min,sec,usec=%d:%d.%d,tsecf=%.3f\n",odoend,min,sec,usec,tsecf);
    if(loopnum!=0){
      timedif = input->DELTA = (odoend - odofirst) / mag_min100;//1000000.0 ;
      if(timedif<0.0){
	timedif+=mag_min6;
	input->DELTA+=mag_min6;
      }
    ...
    printf("odoend=%d,odofirst=%d,timedif=%lf,input->DELTA=%lf,%.3f\n",odoend,odofirst,timedif,input->DELTA,tsecf);
    //    exit(-1);
    for(i=0;i<U->nv;i++) X[i]/=lD;
    if(loopnum == 0){//first time (line) of reading
      input->v=input->omega=input->DELTA=0;
      input->V[0]=input->V[1]=input->V[2]=0;
      input->O[0]=input->O[1]=input->O[2]=0;
    }
    else {
      double dX[3]={X[0]-U->X0[0],X[1]-U->X0[1],X[2]-U->X0[2]};
      double ndX=sqrt(dX[0]*dX[0]+dX[1]*dX[1]+dX[2]*dX[2]);//norm
      for(i=0;i<3;i++) A[i]=dX[i]/ndX;
      double sA[3]={A[0]+U->A0[0],A[1]+U->A0[1],A[2]+U->A0[2]};//sum of A
      double nsA=sqrt(sA[0]*sA[0]+sA[1]*sA[1]+sA[2]*sA[2]);//norm
      for(i=0;i<U->nv;i++){
	input->V[i]=(dX[i])/timedif;//[mm] to [m] <- Velocity
	input->O[i]=(nsA>0)?sA[i]/nsA:0;//[mm] to [m] <- Orientation
      }
      //      input->V[2]*=0.8;//forcing down OK ????
      input->omega=atan2(input->O[1],input->O[0])*R2D/timedif;
      if(1==1){
	//        printf("#omega=%g,%g,%g,time%g\n",input->omega,input->O[0],input->O[1],timedif);
	//        printf("DEBUG1(%d):X(%g,%g,%g),X0(%g,%g,%g)",odotime,X[0],X[1],X[2],U->X0[0],U->X0[1],U->X0[2]);
        printf("#DEBUG2(%d:%d.%d.%d):V(%.1f,%.1f,%.1f),X(%.1f,%.1f,%.1f)",odotime,min,sec,usec,input->V[0],input->V[1],input->V[2],X[0],X[1],X[2]);
      }
    }
//    else{
//      double dX[3]={X[0]-U->X0[0],X[1]-U->X0[1],X[2]-U->X0[2]};
//      double ndX=sqrt(dX[0]*dX[0]+dX[1]*dX[1]+dX[2]*dX[2]);//norm
//      for(i=0;i<3;i++) A[i]=dX[i]/ndX;
//      //      double dA[3]={dX[0]/ndX+U->A0[0],dX[1]/ndX-U->A0[1],dX[2]/ndX-U->A0[2]};
//      /////
//      double sA[3]={A[0]+U->A0[0],A[1]+U->A0[1],A[2]+U->A0[2]};//sum of A
//      double nsA=sqrt(sA[0]*sA[0]+sA[1]*sA[1]+sA[2]*sA[2]);//norm
//      for(i=0;i<U->nv;i++){
//	input->V[i]=(dX[i])/timedif;//[mm] to [m] <- Velocity
//	if(nsA>0) input->O[i]=sA[i]/nsA;//[mm] to [m] <- Orientation
//        else input->O[i]=0;
//      }
//      /////
//      //      for(i=0;i<U->nv;i++) input->V[i]=(dX[i]/1000)/timedif*0.88;//[mm] to [m]
//      //v=(sqrt(dx*dx+dy*dy)/timedif)*0.88;//return value//*0.8//初期値
//      //      for(i=0;i<U->nv;i++) input->V[i]/timedif*0.88;//
//      if(DEBUG!=0){
//        printf("DEBUG1(%d):X(%g,%g,%g),X0(%g,%g,%g)",odotime,X[0],X[1],X[2],U->X0[0],U->X0[1],U->X0[2]);
//        printf("DEBUG2(%d):V(%g,%g),X(%g,%g,%g)",odotime,input->V[0],input->V[1],X[0],X[1],X[2]);
//      }
//      input->omega=atan2(sA[1],sA[2])*R2D/timedif;
//        //      input->omega=(da/timedif)*0.97;//*1.2
//    }
    for(i=0;i<U->nv;i++) U->X0[i]=X[i];
    for(i=0;i<U->nv;i++) U->A0[i]=A[i]; //orientation
  }
  return ret;
  //  return input;
}
char *getInput1(INPUT *input,FILEDATA *U,int gpstime,int loopnum){
  char buff[buffsize];
  char *ret;
  int lD=0,arraynum,flag=0,odofirst=0,beforetime=0;
  int odotime;
  double timedif=0.0;
  int dif[2];
  fpos_t fpos;
  ...
  if(U->class==1){//P3AT by yoshinaga
	...
	a=U->a0;
      ...
      dy/=1000;
      da*=D2R;
      delta_t= (i_min_sec_usec(min,sec,usec)-beforetime) / mag_min100;//1000000.0;
      //      delta_t= (min*1000000+sec*1000 + usec -beforetime) / 1000000.0;
      if(delta_t!=0.0){
	_v=sqrt(dx*dx+dy*dy)/delta_t;
	_omega=da/delta_t;
	v+=_v;omega+=_omega;
      }
      beforetime= i_min_sec_usec(min,sec,usec) ;//min*1000000+sec*1000 + usec;
      U->l++;
    }
    odoend = U->timeA = i_min_sec_usec(min,sec,usec);//min*1000000+sec*1000 + usec;
    if(loopnum!=0){
...
int getORSdatafromfile(double *angORS,FILEDATA *Z,int gpstime,int loopnum,double ang0){//OrientationSensor
  ...
  for(;;){
    ...
    orstime = i_min_sec_usec(min,sec,usec);//min*1000000+sec*1000 + usec;
    if(ln!=0){
      arrynum = (ln + 1) % 2;
      switch(arrynum){
      case 0:
  ...
  double angorg1=ang;
#define ORS_NEW
  *angORS=atan2(sin(ang*D2R),cos(ang*D2R))*R2D; //return value [-pi,pi]=[-180,180]
#ifdef ORS_NEW
#else //#ifdef ORS_NEW
  else{
    *angORS=ang0-ang;
...
int getKinectdatafromfile(KINECTDATA *kinect ,FILEDATA *K,int gpstime,double kinectang0){//OrientationSensor
  ...
  for(;;){
    ...
    //    double min1;
    int dif[2];
    fgets(buff,buffsize,K->fp);
    sscanf(buff,"%lf\t%lf\t%d\t%lf\n",&tmp,&tmp,&min,&secusec);
    //    sscanf(buff,"%lf\t%lf\t%d\t%d.%d\n",&tmp,&tmp,&min,&sec,&usec);
    //    sscanf(buff,"%lf %lf %lf %d.%d\n",&tmp,&tmp,&min1,&sec,&usec);
    //    kinecttime = min*1000000+sec*1000 + usec;
    sec=(int)(secusec);
    usec=(int)(secusec*1000)%1000;
    kinecttime = i_min_sec_usec(min,sec,usec);
    if(ln!=0){
      arrynum = (ln + 1) % 2;
      switch(arrynum){
      case 0:
